...
const packagesData: PackageRow[] = [
    { id: 1, title: 'Bali Bliss & Temples', destination: 'Bali, Indonesia', country: 'Indonesia', duration: '7 Days', price: 1200, rating: 4.8, reviews: 124, image: 'https://images.unsplash.com/photo-1544644181-1484b3fdfc62?auto=format&fit=crop&q=80&w=800', tags: ['Best Seller'], category: 'International', continent: 'asia', styleTags: ['budget', 'nature', 'honeymoon'] },
    { id: 2, title: 'Swiss Alps Adventure', destination: 'Switzerland', country: 'Switzerland', duration: '10 Days', price: 3400, rating: 4.9, reviews: 89, image: 'https://images.unsplash.com/photo-1530122037265-a5f1f91d3b99?auto=format&fit=crop&q=80&w=800', tags: ['Adventure'], category: 'International', continent: 'europe', styleTags: ['adventure', 'luxury', 'nature'] },
    { id: 3, title: 'Maldives Honeymoon', destination: 'Maldives', country: 'Maldives', duration: '5 Days', price: 2800, rating: 5.0, reviews: 210, image: 'https://images.unsplash.com/photo-1514282401047-d79a71a590e8?auto=format&fit=crop&q=80&w=800', tags: ['Honeymoon'], category: 'Honeymoon', continent: 'asia', styleTags: ['honeymoon', 'luxury', 'nature'] },
    { id: 4, title: 'Dubai City Tour', destination: 'UAE', country: 'UAE', duration: '4 Days', price: 900, rating: 4.6, reviews: 340, image: 'https://images.unsplash.com/photo-1512453979798-5ea266f8880c?auto=format&fit=crop&q=80&w=800', tags: [], category: 'International', continent: 'middle-east', styleTags: ['budget', 'luxury', 'group'] },
    { id: 5, title: 'Golden Triangle Explorer', destination: 'India', country: 'India', duration: '8 Days', price: 1100, rating: 4.7, reviews: 156, image: 'https://images.unsplash.com/photo-1524492412937-b28074a5d7da?auto=format&fit=crop&q=80&w=800', tags: ['Culture'], category: 'Domestic', continent: 'india', styleTags: ['family', 'budget', 'nature'] },
    { id: 6, title: 'Santorini Sunset', destination: 'Greece', country: 'Greece', duration: '6 Days', price: 2100, rating: 4.9, reviews: 92, image: 'https://images.unsplash.com/photo-1613395877344-13d4a8e0d49e?auto=format&fit=crop&q=80&w=800', tags: ['Romantic'], category: 'Honeymoon', continent: 'europe', styleTags: ['honeymoon', 'luxury', 'nature'] },
];
...
const travelerInsights = [
    { icon: Utensils, title: 'Food Culture', desc: 'From street food adventures to fine dining — we help you discover the best local flavours at every destination.' },
    { icon: ShoppingBag, title: 'Shopping', desc: 'Know what to buy and where — from local markets to luxury malls, we share insider shopping guides.' },
    { icon: Banknote, title: 'Currency & Budget', desc: 'Smart budgeting tips, currency exchange guidance, and cost-of-living comparisons for every destination.' },
    { icon: Shield, title: 'Safety Tips', desc: 'Updated safety insights, local laws, and practical travel tips so you travel confidently.' },
];
...
export default function Packages() {
    ...
    const filteredPackages = useMemo(() => {
        ...
        if (selectedPrices.length > 0) {
            list = list.filter((p) => selectedPrices.some((b) => matchesPriceBucket(p.price, b)));
        }
        ...
        if (selectedCountries.length > 0) {
            list = list.filter((p) => selectedCountries.some((c) => p.country.toLowerCase() === c.toLowerCase()));
        }

        if (selectedCategories.length > 0) {
            list = list.filter((p) => selectedCategories.some((c) => p.category.toLowerCase() === c.toLowerCase()));
        }

        if (selectedDestinations.length > 0) {
            list = list.filter((p) =>
                selectedDestinations.some((d) => p.destination.toLowerCase().includes(d.toLowerCase()))
            );
        }
        ...
        switch (sortBy) {
            ...
            case 'price-high':
                ...
                break;
            case 'duration-short':
                list.sort((a, b) => packageDayCount(a.duration) - packageDayCount(b.duration));
                break;
            case 'duration-long':
                list.sort((a, b) => packageDayCount(b.duration) - packageDayCount(a.duration));
                break;
            default:
                break;
        }
    ...
    return (
        <div className="bg-slate-50 min-h-screen pb-20 md:pb-0 font-sans">
            <div className="relative min-h-[80svh] flex flex-col items-center justify-center px-6 text-center overflow-hidden">
                <img
                ...
                >
                    ...
                    >
                        From tropical beaches to alpine peaks — find, compare and plan your dream trip with expert guidance
                    </motion.p>
                    ...
                    >
                        <div className="bg-white rounded-2xl shadow-2xl flex items-center gap-3 px-5 py-4">
                            ...
                            <button
                                type="submit"
                                className="min-h-12 bg-primary text-white font-bold px-6 py-2.5 rounded-xl text-sm flex items-center gap-2 shrink-0 transition-colors hover:bg-primary/90"
                            >
                                Search
                            </button>
                        </div>
                    ...
                    {quickInsights.map((insight, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div>
                                    <div className="text-slate-400 text-[11px] uppercase tracking-wide font-bold">{insight.label}</div>
                                    <div className="text-slate-900 font-bold text-sm">{insight.value}</div>
                                </div>
                            </div>
                        );
                    })}
            ...
            <div className="content-container pt-12">
                <div className="text-slate-500 text-xs uppercase tracking-widest font-bold mb-4">Browse by Region</div>
                ...
                >
                    ...
                    {continents.map((cont) => (
                        ...
                        >
                            ...
                            {cont.id !== 'all' && (
                                <span className="text-xs font-bold opacity-60 ml-1">{cont.packageCount} trips</span>
                            )}
                        </button>
                    ))}
                    </div>
            ...
            <div className="content-container mt-8">
                <div className="text-slate-500 text-xs uppercase tracking-widest font-bold mb-4">Filter by Travel Style</div>
            ...
            <div className="content-container mt-12">
                <div className="flex items-center justify-between mb-6">
                    <h2 className="font-['Marcellus'] text-2xl md:text-3xl text-slate-900">Top Destinations Right Now</h2>
                    <a href="/packages" className="text-primary font-semibold text-sm hover:underline flex items-center gap-1 min-h-12">
                ...
                >
                    {featuredDestinations.map((dest) => (
                        <motion.a
                            ...
                            variants={{ hidden: { scale: 0.93, opacity: 0 }, show: { scale: 1, opacity: 1 } }}
                        ...
                        >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                                ...
                                <h3 className="font-['Marcellus'] text-white text-lg font-bold leading-tight">{dest.name}</h3>
                                <p className="text-white/70 text-[11px] font-light leading-snug mt-0.5 hidden md:block">{dest.hook}</p>
                            </div>
                        </motion.a>
                    ))}
                </motion.div>
                    ...
                    <aside className={`lg:w-1/4 ${showFilters ? 'block' : 'hidden lg:block'}`}>
                        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 sticky top-28">
                            <div className="flex items-center justify-between mb-6 pb-4 border-b border-slate-100">
                                ...
                                <button type="button" onClick={clearAllFilters} className="text-primary text-xs font-bold hover:underline cursor-pointer min-h-10 px-1">
                                    Reset
                                </button>
                            </div>
                    ...
                    <main className="lg:w-3/4">
                        <div className="flex items-center justify-between mb-6 flex-wrap gap-4 sticky top-16 lg:top-24 bg-slate-50/90 backdrop-blur-md z-20 py-2">
                            <div className="flex items-center gap-3 flex-wrap">
                                <span className="text-slate-600 text-sm">Showing {filteredPackages.length} packages</span>
                                {hasActiveFilters && (
                                    <div className="flex gap-2 flex-wrap items-center">
                                        ...
                                        {sortBy !== 'recommended' && (
                                            <span className="bg-primary/10 text-primary text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5">
                                                Sort: {sortOptions.find((o) => o.value === sortBy)?.label.replace(/^Sort by: /, '')}
                                                <button
                                                    type="button"
                                                    className="min-h-8 min-w-8 flex items-center justify-center rounded-full hover:bg-primary/20"
                            ...
                            <Dropdown options={sortOptions} value={sortBy} onChange={setSortBy} size="sm" className="w-[200px]" />
                        </div>
                                        ...
                                        <div className="p-5">
                                            ...
                                            <div className="flex items-center gap-4 text-slate-400 text-xs mb-4">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span className="text-slate-900 font-semibold">{pkg.rating}</span>
                                            ...
                                            <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                                                <div>
                                                    ...
                                                    <div className="font-['Marcellus'] text-2xl text-slate-900">${pkg.price}</div>
                                                </div>
                                                <a
                                                    href="/contact"
            ...
            <div className="bg-slate-900 py-20 mt-20">
                <div className="content-container">
                    <h2 className="font-['Marcellus'] text-3xl md:text-4xl text-white text-center mb-3">Traveller Insights</h2>
                    <p className="text-white/55 font-light text-center mb-12">Everything you need to know before you go</p>
                        ...
                        {travelerInsights.map((insight) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <h3 className="font-['Marcellus'] text-xl text-white font-bold mb-3">{insight.title}</h3>
                                    <p className="text-white/60 text-sm leading-relaxed font-light">{insight.desc}</p>
                                </motion.div>
                            );
                        })}
                    </motion.div>
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start md:items-center justify-between">
                        <div className="md:w-1/2">
                            ...
                            <p className="text-white/75 font-light text-lg">
                                Our experts craft personalised itineraries tailored to your travel style, budget and dream.
                            </p>
                        </div>
